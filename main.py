...
from db_api import dbConnect, createTable, prepare_data_query
import pandas as pd
...
if __name__ == '__main__':
    ...
    if oz_downloader.result_fbs:
        prepare_data_query(conn, 'orders', oz_postings_fbs, 'products', oz_products_fbs)
    ...
    if oz_downloader.result_fbo:
        prepare_data_query(conn, 'orders', oz_postings_fbo, 'products', oz_products_fbo)
    ...
    if wb_downloader.result_fbo:
        prepare_data_query(conn, 'orders', wb_orders_fbo, 'products', wb_products_fbo)

    wb_orders_fbs, wb_products_fbs = wb_downloader.get_orders_fbs(get_date(week=4))
    if wb_downloader.result_fbs:
        prepare_data_query(conn, 'orders', wb_orders_fbs, 'products', wb_products_fbs)
    ...
    if ym_downloader.result:
        prepare_data_query(conn, 'orders', ym_orders, 'products', ym_products)
